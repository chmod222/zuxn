...
pub const identifier_length = 64;

pub const Label = [identifier_length:0]u8;

line: usize = 1,
column: usize = 1,

macro_names: [0x100]Label = [1]Label{[1:0]u8{0x00} ** identifier_length} ** 0x100,
...
fn parse_hex_digit(octet: u8) Error!u4 {
...
fn parse_hex_literal(comptime T: type, raw: []const u8, fixed_width: bool) Error!T {
    if (fixed_width) {
...
fn read_label(scanner: *@This(), input: anytype) Error!Label {
    const label = try scanner.read_whitespace_delimited(identifier_length, input);
...
pub fn read_token(scanner: *@This(), input: anytype) Error!?SourceToken {
    ...
    while (scanner.read_byte(input)) |b| {
        ...
        const token: Token = switch (b) {
            ...
            '|', '$' => b: {
                // Padding (TODO)
                var pad = try scanner.read_whitespace_delimited(identifier_length, input);
            ...
            '%' => b: {
                const ident = try scanner.read_whitespace_delimited(identifier_length, input);
            ...
            '"' => b: {
                ...
                var needle = [1:0]u8{b} ++ [1:0]u8{0x00} ** (identifier_length - 1);
                var remain = try scanner.read_whitespace_delimited(identifier_length, input);
